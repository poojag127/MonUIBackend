...
public class MonitorConfigurationUtility
{
  ...
  /***
   * 
   * This method return 1. object for treetable component = treeTableData 2.
   * list of Tiers = tierList
   * 
   * 
   * @param topoName
   * @param jsonName
  ...
  /*
   * Get create configuration json from standard monitor file getMode = 0 all
   * come 1 all come and hide user monitor (remove hidden monitor from object) 2
   * all come with mark hidden mark
   */
  public HashMap<String, ArrayList<MonitorsDTO>> getMonInfoListWithUserFilter(String userName, byte getMode)
  {
    Log.debugLogAlways(ClassName, "getMonTierTableData", "", "",
        "Method called. user Name = " + userName + ", Hide user hiddem monittor or not = " + getMode);
    ...
    try
    {
      ...
      {
        Log.errorLog("MonitorWebService", "getMonTierTableData", "", "",
            "Standard Monitor does not exists = " + stdMonFilePath);
        return filerMonMap;
      }
  ...
  private void printData(HashMap<String, ArrayList<MonitorsDTO>> filerMonMap)
  {
    ...
    for (Map.Entry<String, ArrayList<MonitorsDTO>> keywordEntry : filerMonMap.entrySet())
    {
      ...
      {
        str = str + " " + perGrpData.get(i).getMonName() + "//" + perGrpData.get(i).getDescription() + "//"
            + perGrpData.get(i).getDrivenJsonName() + "\n";
      }
  ...
  public ArrayList<TreeTableDTO> createTreeTableData(ArrayList<TierDTO> tierList,
      HashMap<String, ArrayList<MonitorsDTO>> filerMonMap)
  {
  ...
  public List<ChildNodeDataDTO> getLeafNodeByCategoryName(String topoName, String profileName, String categoryName,
      String userName)
  {
    ArrayList<TierDTO> tierList = getTierList(topoName);
